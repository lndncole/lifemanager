...
async function startChat(conversation) {
  ...
  try {
    ...
    console.log("hello");
    const assistant = openai.beta.assistants.create(conversationObject);

    const query = {
      role: "user",
      content: "Tell me what the weather is like in London."
    };
    ...
    const retrieval = openai.beta.threads.runs.retrieve(thread.id, run.id);
